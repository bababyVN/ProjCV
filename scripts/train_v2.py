# =============================================================
#  train_v2.py — SwinFAN-v2 Training & Validation Loop
#
#  Run with: python scripts/train_v2.py
#
#  Differences from train.py (SwinFAN-v1):
#    • ARCH = "swinfan_v2"  — Swin-Base backbone (88M params)
#    • BATCH_SIZE = 4       — reduced to fit Kaggle T4 VRAM
#    • Gradient accumulation every GRAD_ACCUM steps (effective
#      batch size = BATCH_SIZE × GRAD_ACCUM = 8)
#    • Loss = SwinFANv2Loss (Focal 30% + Dice 30% + Lovász 40%)
#    • Checkpoint saved to output/best_model_v2.pth
#    • Reports both 7-class mIoU (official) and 6-class mIoU
#      (academic standard, excluding noisy "Unknown" class)
#
#  Project structure:
#      config.py                 — base hyperparameters & class maps
#      dataloader/dataloader.py  — DeepGlobeDataset, get_dataloaders
#      encoder/swin_encoder.py   — SwinBaseEncoder (Swin-B backbone)
...
# ─────────────────────────────────────────────────────────────
# SwinFAN-v2 Configuration Overrides
# ─────────────────────────────────────────────────────────────
...
V2_EPOCHS     = 50     # more epochs to benefit from larger model capacity
V2_BATCH_SIZE = 4      # reduce physical batch to fit T4 VRAM (16 GB)
V2_LR         = 3e-4   # slightly lower LR for Swin-B (larger model)

CFG_V2 = copy.deepcopy(CFG)
CFG_V2["ARCH"]       = "swinfan_v2"
CFG_V2["BATCH_SIZE"] = V2_BATCH_SIZE
CFG_V2["EPOCHS"]     = V2_EPOCHS
CFG_V2["LR"]         = V2_LR
CFG_V2["ETA_MIN"]    = 1e-6
CFG_V2["BEST_MODEL"] = str(Path(CFG["OUTPUT_DIR"]) / "best_model_v2.pth")


# ─────────────────────────────────────────────────────────────
# Mixed-precision helpers (same as train.py)
# ─────────────────────────────────────────────────────────────
...
def auto_detect_dataset_root(configured_path: str) -> str:
    ...
    print("[train_v2] Configured DATA_ROOT not found. Auto-detecting...")
    for root, dirs, files in os.walk("/kaggle/input"):
        if "train" in dirs:
            train_path = Path(root) / "train"
            if list(train_path.glob("*_sat.jpg")):
                print(f"[train_v2] Detected dataset root: {root}")
                return root
...
def main():
    ...
    data_root = auto_detect_dataset_root(CFG_V2["DATA_ROOT"])
    CFG_V2["DATA_ROOT"] = data_root

    # ── Setup ──────────────────────────────────────────────────
    seed_everything(CFG_V2["SEED"])
    device     = CFG_V2["DEVICE"]
    task       = CFG_V2["TASK"]
    output_dir = Path(CFG_V2["OUTPUT_DIR"])
    output_dir.mkdir(parents=True, exist_ok=True)
    best_ckpt  = Path(CFG_V2["BEST_MODEL"])

    print("=" * 60)
    print(" Segmentation — SwinFAN-v2 (Swin-Base)")
    print("=" * 60)
    print(f"  Device            : {device}")
    if device == "cuda":
        print(f"  GPU               : {torch.cuda.get_device_name(0)}")
        print(f"  CUDA              : {torch.version.cuda}")
    print(f"  Task              : {task}  ({CFG_V2['NUM_CLASSES']} classes)")
    print(f"  Epochs            : {CFG_V2['EPOCHS']}")
    print(f"  Batch size        : {CFG_V2['BATCH_SIZE']} × {GRAD_ACCUM} steps"
          f" = {CFG_V2['BATCH_SIZE'] * GRAD_ACCUM} effective")
    print(f"  LR                : {CFG_V2['LR']}")
    ...
    # ── Data ───────────────────────────────────────────────────
    print("\n[1/4] Preparing data...")
    df               = build_dataframe(CFG_V2["DATA_ROOT"])
    train_df, val_df = split_dataframe(df, CFG_V2["VAL_SPLIT"], CFG_V2["SEED"])
    train_loader, val_loader = get_dataloaders(train_df, val_df, CFG_V2)
    print(f"  Train batches : {len(train_loader)}")
    print(f"  Val   batches : {len(val_loader)}")

    # ── Model ──────────────────────────────────────────────────
    print("\n[2/4] Building model...")
    model = build_model(CFG_V2).to(device)
    ...
    for epoch in range(1, CFG_V2["EPOCHS"] + 1):
        print(f"{'-' * 60}")
        print(f"  Epoch [{epoch:02d}/{CFG_V2['EPOCHS']:02d}]"
